...
Functions:
    change_dns(network_interface, dns1, dns2): Temporarily changes the DNS to avoid request failures. (Windows only, need admin privileges)
...
# Need to change DNS temporarily or requests will fail
def change_dns(network_interface, dns1, dns2):
    command1 = f"netsh interface ip set dns name=\"{network_interface}\" static {dns1}"
    command2 = f"netsh interface ip add dns name=\"{network_interface}\" addr={dns2} index=2"
...
def get_public_ip():
    try:
        change_dns("Wi-fi", "1.1.1.1", "1.0.0.1")
        ...
        change_dns("Wi-fi", "192.168.0.70", "1.1.1.1")
